...
class Game:
    ...
    def placePieces(self):
        ...
        for i in range(0,8):
            ...
            #for put the pwan in right place
        placers = [Rook,Knight,Bishop,Queen,King,Bishop,Knight,Rook]
        #array of rest soliders
        for i in range(0,8):
            self.gameboard[(i,0)] = placers[i](WHITE,uniDict[WHITE][placers[i]])
            self.gameboard[((7-i),7)] = placers[i](BLACK,uniDict[BLACK][placers[i]])
        placers.reverse()
        #put on the board the others soliders
...
class Pawn(Piece):#for pawn to check the moves he can go 
...
#for define by dictenory the soliders and how thay look 
Game()
#for start class game
